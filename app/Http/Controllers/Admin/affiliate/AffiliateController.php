...
class AffiliateController extends Controller
{
    ...
    {
        $verification_form = AffiliateConfig::where('type', 'verification_form')->first();
        if (! $verification_form) {
            $verification_form = new AffiliateConfig;
            $verification_form->type = 'verification_form';
            $verification_form->value = json_encode([
                [
                    'type' => 'text',
                    'label' => 'Name',
                ],
                ...
                [
                    'type' => 'file',
                    'label' => 'Verification Document (NID/Passport)',
                ],
            ]);
            $verification_form->save();
        }

        $validation_time = AffiliateConfig::where('type', 'validation_time')->first();
        if (! $validation_time) {
            $validation_time = new AffiliateConfig;
            $validation_time->type = 'validation_time';
            $validation_time->value = '30';
    ...
    public function apply_for_affiliate(Request $request)
    {
        ...
        //     return back();
        // }
        return view('Frontend.affiliate.apply_for_affiliate');
    }
    ...
    public function user_index(Request $request)
    {
        ...
        } elseif ($request->type == '7' || $request->type == '30') {
    ...
    public function processAffiliatePoints(Orders $order)
    {
