...
@section('content')
    <style>
        ...
        /* New Modern Progress Style Container */
        .progress-rates-box {
            display: flex;
        ...
        /* Custom dynamic gauge borders colors matching your specific color standard */
        .progress-item-wrapper:nth-child(1) .bar { border-bottom-color: #6366f1; border-right-color: #6366f1; }
        .progress-item-wrapper:nth-child(2) .bar { border-bottom-color: #f59e0b; border-right-color: #f59e0b; }
        .progress-item-wrapper:nth-child(3) .bar { border-bottom-color: #22c55e; border-right-color: #22c55e; }
        .progress-item-wrapper:nth-child(4) .bar { border-bottom-color: #ef4444; border-right-color: #ef4444; }
    ...
    <div class="row">
        ...
        <div class="col-lg-3 col-sm-6 mb-4">
            <a href="{{ route('order-delivered') }}" class="stat-widget-premium"
                ...
                <div class="card-body">
                    <div class="row d-flex justify-content-evenly pb-4">
                        ...
                        <div class="progress">
                            ...
                            <span>{{ number_format($confirmationRate, 1) }}</span>%
                        </div>
                        <div class="progress-rate-title">Confirmation Rate</div>
                    </div>
                        </div>
                        <div class="col-lg-3 col-md-6 col-sm-6">
                            <div class="progress-item-wrapper">
                        ...
                        <div class="progress-rate-title">Pending Rate</div>
                    </div>
                        </div>
                        <div class="col-lg-3 col-md-6 col-sm-6">
                            <div class="progress-item-wrapper">
                        <div class="progress">
                            ...
                            <span>{{ number_format($deliveryRate, 1) }}</span>%
                        </div>
                        <div class="progress-rate-title">Delivery Rate</div>
                    </div>
                        </div>
                        <div class="col-lg-3 col-md-6 col-sm-6">
                            <div class="progress-item-wrapper">
                        <div class="progress">
                            ...
                            <span>{{ number_format($cancelRate, 1) }}</span>%
                        </div>
                        <div class="progress-rate-title">Cancel Rate</div>
                    </div>
                        </div>
                    </div>
                </div>
                            ...
                            <tbody>
                                ...
                                @forelse ($latestOrder as $order)
                                    @php
                                        ...
                                        if (in_array(strtolower($order->delivery_status), ['delivered', 'success'])) {
                                            $badgeClass = 'badge-soft-success';
                                        }
                                        if (in_array(strtolower($order->delivery_status), ['cancel', 'canceled'])) {
                                            $badgeClass = 'badge-soft-danger';
                                        }
                                        if (in_array(strtolower($order->delivery_status), ['pending', 'hold'])) {
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center py-4 text-muted">No recent orders found.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                    ...
                    <div class="table-responsive">
                        <table class="table-modern">
                            ...
                            <tbody>
                                        ...
                                        <td>
                                            <div class="d-flex flex-column">
                                                <span class="fw-bold text-truncate"
                                                    style="max-width: 150px;">{{ $topSellingProduct->title }}</span>
                                                <span class="text-muted small">Code:
                                                    LM-{{ $topSellingProduct->code }}</span>
                                            </div>
                                        </td>
                                        <td>
                                            <span
                                                class="fw-bold {{ $topSellingProduct->stock < 10 ? 'text-danger' : 'text-success' }}">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="text-center py-4 text-muted">No top products available.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
...
@section('script')
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            // 1. Semi Circle Progress Gauges Animation
            $(".progress").each(function() {
                var $bar = $(this).find(".bar");
                var $val = $(this).find("span");
                var perc = parseFloat($val.text()); // float used to parse rate precisely (e.g. 56.5%)
            ...
            const centerTextPlugin = {
                ...
                beforeDraw: function(chart) {
                    ...
                    ctx.font = "bold " + Math.max(14, Math.min(24, fontSize * 18)) +
