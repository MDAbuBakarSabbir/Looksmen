...
<script>
    ...
    document.querySelectorAll('.status-switch').forEach(function(btn) {
        btn.addEventListener('change', function() {
            ...
            .then(data => {
                if (data.success) {
                    Toast.fire({ icon: 'success', title: status == 1 ? 'Slider Activated' : 'Slider Deactivated' });
                } else {
    ...
    addSliderBtn.onclick = () => sliderModal.classList.add('show');
    cancelBtn.onclick = () => {
    ...
    window.onclick = (e) => { if (e.target == sliderModal) cancelBtn.onclick(); };
    ...
    document.getElementById('addSliderForm').addEventListener('submit', function(e) {
        ...
        .then(result => {
            if (result.success) {
                ...
                Toast.fire({ icon: 'error', title: 'Failed to save slider' });
                submitBtn.disabled = false;
                submitBtn.innerHTML = 'Save Slider';
            }
        })
        .catch(err => {
