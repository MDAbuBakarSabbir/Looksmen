...
<head>
    ...
    @if (!empty($webConfig['fb_pixel']))
        {!! $webConfig['fb_pixel'] !!}
    @elseif (!empty($fbPixelId))
        <!-- Automated Meta Pixel & DataLayer Deduplication Engine -->
        <script>
            ...
            // Disable automatic button scrapers to avoid duplicate SubscribedButtonClick
            ...
            // --- Automated DataLayer to Meta Pixel Bridge (Zero GTM Manual Setup Required) ---
            (function() {
                window.dataLayer = window.dataLayer || [];
                ...
                function handleDataLayerItem(item) {
                    ...
                    var eventId = item.event_id || (item.ecommerce && item.ecommerce.event_id);

                    // Deduplicate within browser execution
                    var uniqueKey = eventName + '_' + (eventId || JSON.stringify(item));
                    if (_processedEvents[uniqueKey]) return;
                    _processedEvents[uniqueKey] = true;

                    var options = eventId ? { eventID: eventId } : {};

                    if (eventName === 'view_item') {
                        var ecom = item.ecommerce || {};
                        var items = ecom.items || [];
                        var firstItem = items[0] || {};
                        var val = item.value || ecom.value || firstItem.price || 0;
                        var prodId = firstItem.item_id || item.item_id || (item.content_ids ? item.content_ids[0] : '');
                        var prodName = firstItem.item_name || item.content_name || '';

                        fbq('track', 'ViewContent', {
                            content_type: 'product',
                            content_ids: prodId ? [String(prodId)] : [],
                            content_name: prodName,
                            value: parseFloat(val) || 0,
                            currency: item.currency || ecom.currency || 'BDT'
                        }, options);
                    }
                        ...
                        var items = ecom.items || [];
                        var firstItem = items[0] || {};
                        var val = item.value || ecom.value || (firstItem.price * (firstItem.quantity || 1)) || 0;
                        var prodId = firstItem.item_id || item.item_id || (item.content_ids ? item.content_ids[0] : '');
                        var prodName = firstItem.item_name || item.content_name || '';
                        var qty = firstItem.quantity || item.quantity || 1;
                        ...
                        fbq('track', 'AddToCart', {
                            ...
                            contents: [{
                                ...
                                item_price: parseFloat(firstItem.price || val) || 0
                            }],
                            value: parseFloat(val) || 0,
                            currency: item.currency || ecom.currency || 'BDT',
                            num_items: parseInt(qty) || 1
                        }, options);
                    }
                    else if (eventName === 'begin_checkout') {
                        var ecom = item.ecommerce || {};
                        var items = ecom.items || [];
                        var val = item.value || ecom.value || 0;
                        var contentIds = [];
                        var contents = [];
                        ...
                        if (Array.isArray(items)) {
                            items.forEach(function(it) {
                                if (it && it.item_id) {
                                    contentIds.push(String(it.item_id));
                                    contents.push({
                        ...
                        if (item.customer_phone || item.customer_email || item.customer_name) {
                            fbq('setUserProperties', {
                                'em': item.customer_email ? item.customer_email.toLowerCase().trim() : '',
                                'ph': item.customer_phone ? String(item.customer_phone).replace(/[^0-9]/g, '') : '',
                                'fn': item.customer_name ? item.customer_name.toLowerCase().trim() : ''
                            });
                        }

                        fbq('track', 'InitiateCheckout', {
                            content_type: 'product',
                            content_ids: contentIds.length > 0 ? contentIds : (item.content_ids || []),
                            contents: contents,
                            value: parseFloat(val) || 0,
                            currency: item.currency || ecom.currency || 'BDT',
                            num_items: items.length || item.num_items || 1
                        }, options);
                    }
                        ...
                        var items = ecom.items || [];
                        var val = item.value || ecom.value || 0;
                        var orderId = item.transaction_id || ecom.transaction_id || '';
                        var contentIds = [];
                        var contents = [];
                        ...
                        var customer = item.customer || item.user_data || {};
                        var phone = item.customer_phone_international || item.customer_phone || customer.phone || customer.phone_number || '';
                        var email = item.customer_email || customer.email || '';
                        var firstName = item.customer_first_name || (customer.address ? customer.address.first_name : null) || customer.first_name || item.customer_name || '';
                        var lastName = item.customer_last_name || (customer.address ? customer.address.last_name : null) || customer.last_name || '';
                        var city = item.customer_city || customer.city || (customer.address ? customer.address.city : '') || '';
                        var district = item.customer_district || customer.district || (customer.address ? customer.address.region : '') || '';

                        fbq('setUserProperties', {
                            'fn': firstName ? firstName.toLowerCase().trim() : '',
                            'ln': lastName ? lastName.toLowerCase().trim() : '',
                            'em': email ? email.toLowerCase().trim() : '',
                            'ph': phone ? String(phone).replace(/[^0-9]/g, '') : '',
                            'ct': city ? city.toLowerCase().trim() : '',
                            'st': district ? district.toLowerCase().trim() : 'bd',
                            'country': 'bd',
                            'external_id': customer.id && customer.id !== '0' ? String(customer.id) : ('order_' + orderId)
                        });
                ...
                if (Array.isArray(window.dataLayer)) {
                    for (var i = 0; i < window.dataLayer.length; i++) {
                        handleDataLayerItem(window.dataLayer[i]);
                    }
                }

                // Hook dataLayer.push in real time
                var originalPush = window.dataLayer.push;
                window.dataLayer.push = function() {
                    var res = originalPush.apply(window.dataLayer, arguments);
                    for (var j = 0; j < arguments.length; j++) {
                        handleDataLayerItem(arguments[j]);
                    }
                    return res;
                };
            })();
